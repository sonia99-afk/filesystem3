...
/* =========================
   ...
   - Shift/Alt/Ctrl/Cmd are treated as NORMAL keys (not special modifiers)
   - layout-independent for letters/digits via e.code
   - IMPORTANT FIXES:
     * macOS Command is reported as key="Meta" in browsers -> we normalize it to "Control" ONLY on Apple.
       (So configs can stay "Control+Z" etc., while UI can display it as "Command".)
     * macOS CapsLock may produce key="Unidentified" on keyup -> we normalize CapsLock via e.code
       and never put "Unidentified" into combos.
   ========================= */
...
function normalizeKeyTokenFromEvent(e) {
  ...
  // ---- Stable special cases first ----
  // CapsLock on macOS can report key="Unidentified" on keyup. Use e.code.
  ...
  // If browser couldn't identify the key name, prefer code (if any) over "Unidentified".
  if (key === "Unidentified") return code || "";

  if (!key) return "";

  // Normalize a few historical aliases
  if (key === " " || key === "Spacebar") return "Space";
  if (key === "Esc") return "Escape";

  // Make Shift+'=' case stable: we record '+' as Plus
  if (key === "+") return "Plus";
  ...
  // Keep physical Ctrl as "Control" everywhere
  if (key === "Control") return "Control";

  // macOS Command is key="Meta". We store it as "Control" so configs match across platforms.
  // On Windows we DO NOT remap Win-key, to avoid breaking existing behavior.
  if (IS_APPLE && (key === "Meta" || key === "OS")) return "Control";
...
function shouldTrackPressed() {
  const ae = document.activeElement;

  // In text fields / rename inputs / export mode we don't track chords.
...
window.addEventListener(
  "keydown",
  (e) => {
    if (!shouldTrackPressed()) {
      clearPressed();
      return;
    }

    // don't mess with tab/escape
...
window.addEventListener(
  "keyup",
  (e) => {
    // If we are in an input/edit context — force clear to avoid "stuck" combos.
    if (!shouldTrackPressed()) {
      clearPressed();
      return;
    }
...
window.addEventListener("focus", clearPressed);
document.addEventListener("visibilitychange", () => {
  if (document.hidden) clearPressed();
});

function comboFromPressed() {
...
function isHotkey(e, action) {
  ...
  // In your app hotkeys are single-shot; held-repeat is handled elsewhere.
  if (e.repeat) return false;

  const haveRaw = comboFromPressed();
