...
/* =========================
   Hotkeys (stable, no sticky)
   - Mod = Ctrl (Windows/Linux) or Command (macOS)
   - combo фиксируется по одному KeyboardEvent (keydown), без отслеживания Set нажатых
   ========================= */

function isHotkey(e, action) {
  if (!window.hotkeys?.matchEvent) return false;
  if (e.repeat) return false;
  return window.hotkeys.matchEvent(action, e);
}
